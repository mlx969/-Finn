...
bullets(s, [
    ...
    "双版本架构：18+ 版（升维对话 + 小众文化科普）与未成年版（适龄科普 + 游戏化互动，零成人内容）。18+ 论坛与成人展资讯为后续扩展，不在参赛教育主线。",
    "在已检索的公开样本中，本项目是把「升维视角 + 适龄双版本 + 安全护栏」整合到同一开源方案的较少见尝试；中文性教育/亲密关系垂类公开项目很少（海外仅 5 个且只做生理问答）。",
...
bullets(s, [
    ...
    ("洞察：青少年普遍早熟但概念最模糊，处于「好奇+追求刺激+不懂后果」高风险区；现有轮子几乎全 18+ 成人向，适龄段在我已检索的公开样本中较少见。", 0),
...
bullets(s, [
    ...
    "性教育垂类公开样本极少：开源「性健康 chatbot」仅 5 个、全在海外、且全部只做生理问答（已检索公开样本，非穷尽证明全球唯一）。",
    "竞品共性缺失：SafeBubble(丹麦,隐私即架构) / TroyHealthLink(美,风险自评) / Aarogya_Mitram(印,8 Agent) … 都无「升维+社区+适龄版」。",
    "结论：技术水位≈硕士毕设，Dify/FastGPT 低代码 3–7 天可复刻；本项目是把「升维+双版本+社区+审核」整合到同一开源方案的较少见尝试（公开样本较少见，非穷尽证明全球唯一）。",
], Inches(0.5), Inches(1.4), Inches(12.3), Inches(5.4), size=15, gap=11)
footer(s)
...
bullets(s, [
    ...
    "五层架构：①知识层(RAG权威资料) ②升维层(情感社会学框架) ③多元层(SM/性少数去污名,consent前置) ④社群层(18+论坛+展讯,后续扩展) ⑤适龄层(未成年版动漫/游戏化,独立应用不挂成人库)。",
...
bullets(s, [
    ...
    "实现：身份/年龄认证后分流，未成年版与成年版为两个独立 Dify 应用，未成年版不挂成人知识库（数据与内容隔离，详见第9章）。",
...
bullets(s, [
    ...
    "技术栈：阶跃 StepFun(step-router-v1/Step3.7 Flash) + Dify Chatflow(主力,可私有化) + FastGPT(对照) + 规划中的 ShieldLM-6B+LlamaGuard S1–S13+DFA(四层审核) + 单文件index.html + Cloudflare Pages(部署)。",
], Inches(0.5), Inches(1.4), Inches(12.3), Inches(5.4), size=14, gap=10)
footer(s)

# 9. 知识库
s = slide(); fill_bg(s)
title_bar(s, "05 · 知识库构建", "权威来源 × 双库隔离 × 原创重写", "09")
bullets(s, [
    "权威来源：WHO 性健康与生殖健康指南、中国计生协/疾控公开科普、多元性文化去污名化学术综述。",
    "双库隔离：kb_adult(WHO/计生协/疾控+多元+升维,18+版,已在Dify创建) / kb_minor(刘文利读本/UNESCO ITGSE/保护豆豆/LoveMatters青春版/青爱工程,未成年版,规划中,先12–15)，未成年版应用不挂成人知识库。",
...
# 10. 安全与合规（核心）
s = slide(); fill_bg(s)
title_bar(s, "06 · 安全与合规 ★", "18+ 硬门槛 + 七条强制规则 + 规划中的四层审核", "10")
bullets(s, [
    ...
    "规划中的四层审核：L1 DFA敏感词(6万,毫秒拦截) / L2 ShieldLM-6B(语义二审) / L3 人工抽审+危机升级Agent / L4 用户信用+举报。",
    "内容分级规划采用 Llama Guard MLCommons S1–S13：S4(儿童性剥削)零容忍硬拦，S3/S10/S11 全覆盖高危场景（规划，未上线）。",
...
# 11. 社区与论坛
s = slide(); fill_bg(s)
title_bar(s, "07 · 社区与论坛（规划·18+版）", "四版块 + 规划中的四层审核 + 成人展聚合", "11")
bullets(s, [
    "四个版块：①升维讨论(情感模式与原生家庭/阶层) ②安全教育(SM/多元文化科普) ③成人展资讯(公开活动聚合) ④互助答疑(经验分享)。",
    "门槛：18+ 强制 + 规划中的四层审核；小众文化区 NSFW 折叠 + 二次年龄验证(规划)。",
...
bullets(s, [
    ...
    "测试集 152 条(已生成)：知识31 / 升维38 / 安全37 / 拒答·转介46，四类均衡（计数由 gen_eval_counts.py 实算）。",
    "红队 47 条(已生成)：越狱/色情诱导/伪装未成年/违法诱导/指令注入/PUA/对抗绕审/概念扭曲，每条标攻击类型与预期拦截层。",
    "五维评分 Rubric(0–2)：知识形式合规 / 升维质量 / 安全合规 / 拒答恰当 / 引用规范；红队以拦截率为核心指标。",
...
bullets(s, [
    ...
    "开源价值：公开升维视角提示词框架 + 规划中的四层审核架构 + 评测/红队集 + 双版本适龄分级范式，降低领域安全研究门槛。",
...
bullets(s, [
    "内容合规(未成年人/色情)：高 → 18+硬门槛+规划中四层审核+S4零容忍(规划)+双版本数据与内容隔离。",
...
_set(tf, "可体验 · 可复制(MIT 开源) · 可落地 —— 在已检索的公开样本中，较少见的「升维+双版本+社区+合规审核」整合开源尝试。", 15, GREY, space_after=10)
