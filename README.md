...
| 维度 | 说明 |
|---|---|
| **学习者是谁** | 分两条线：① 青少版——未满 18 岁，做身体权启蒙与健康边界；② 成年版——18+，做性健康与亲密关系科普 |
| **要学会什么** | 身体权与边界、性同意、避孕与防病、危机识别与求助；以及对情感困惑的结构性（原生家庭 / 阶层 / 身份）理解 |
| **AI 在教学中做什么** | 年龄分流后，按适龄课程给出要点，用「1 卡 → 情境 → 3 题测验 → 反馈」的闭环帮助自学；RAG 接入权威源做问答；做安全检测与危机转介 |
| **如何验证学会了** | 首页「学习进度」面板实时展示**已完成课程 / 测验平均正确率**（本地数据）；「风险识别前后测」为**待验证指标**（无规模化实测前不造假） |

> ⚠️ 当前为**高保真交互原型**：首页问答与课程测验是**预设的本地演示样例（非实时 AI）**；实时 AI 需接入已部署且经评审的 Dify 应用（参赛版暂未开放公网嵌入）。评测为**正则启发式初筛 + 人工复核**，非裁判模型得分。
...
但 **性教育垂类的中文开源项目很少**：在我检索过的公开样本中，专门做中文性健康教育的开源项目极少，
且国外 5 个相关项目（SafeBubble / TroyHealthLink / sex-education-chatbot-backend / Aarogya_Mitram / SuSastho.AI）全部只做"生理问答"，
**没有社会学升维、没有社区、没有审核**。把「升维视角 + 适龄双版本 + 安全护栏」整合到同一套开源方案里，在我已检索的公开样本中较少见。

> 注：上述「较少见」是基于本项目截至 2026-08-15 的公开检索结论，不能穷尽证明「全球唯一 / 全球空白」。
...
- **安全护栏（规划中）**：当前 MVP 以**提示词七条规则 + 危机白名单转介 + 青少版关键词第一层词面初筛**兜底；DFA / ShieldLM / Llama Guard 四层审核为后续规划（详见报告 §6.3）。
...
3. 成年版 / 青少版均提供**预设的本地演示问答样例**（明确标注「非实时 AI」），演示教育辅助的交互形态；
4. 扩展内容（成人健康展讯）为独立页 `adult_expo.html`，仅外链官方资源，**非参赛教育主线**。

> 真实 AI 接入说明：本页问答当前为**预设本地演示样例**，非实时模型。若已部署并经评审的 Dify 应用开启嵌入，可在 `index.html` 的 `CONFIG.USE_LIVE_DIFY` 中填入真实地址启用（参赛版暂未开放公网嵌入，且 Dify Cloud 处理对话时数据保存策略以 Dify 平台为准）。**页面无「实时 Key 输入」控件**，Key 仅从环境变量读取，绝不写入文件。
