...
block("我们解决什么", [
    ...
    "适龄段空白：现有性健康 bot 几乎全 18+ 成人向，未成年版(12–18)在公开样本中极少占；青少年早熟≠懂，处于高风险区。",
])
block("差异化定位（核心评分点）", [
    "做「第四类」：知识 + 结构视角 + 适龄分级 + 安全 + 社区；不做教科书/猎奇/PUA 物化。",
    "五层架构：知识 → 升维 → 多元(去污名,consent前置) → 社群(18+论坛+展讯,规划) → 适龄(未成年版动漫/游戏化,独立应用不挂成人库)。",
    "双版本壁垒：身份/年龄认证分流，后端两个独立应用、未成年版不挂成人知识库；绝不性化未成年人，未成年版零色情/零性暗示/零SM。",
])
block("系统架构 & 已跑通", [
    "成年版 Chatflow 5 节点链路已在 Dify 实跑：用户输入 → kb_adult 检索(TopK=3) → 条件分支 → 阶跃 Step 3.7 Flash + 7条规则 → 回复(含来源)。",
    "技术栈：阶跃 StepFun + Dify Chatflow(主力,可私有化) + FastGPT(对照) + 规划中的 ShieldLM-6B/LlamaGuard S1–S13/DFA(四层审核) + 单文件 index.html + Cloudflare Pages。",
])
block("安全合规（核心评分点）", [
    "18+ 硬门槛(第一道墙) + 七条强制安全规则(AGE_GATE/NO_PORN/CONSENT_FIRST/SELF_HARM/VIOLENCE/MEDICAL/SOURCE_TAG)。",
    "规划中的四层审核 + Llama Guard S4(规划采用,儿童性剥削)零容忍硬拦；危机转介用已核实热线白名单(12356/12338/12355/110/120)，设过期复核。",
])
block("评测方案（核心评分点）", [
    "三组对照：A 裸模型 / B Dify版(主方案) / C FastGPT版；测试集 152 条(知识31/升维38/安全37/拒答转介46)；红队 47 条。",
...
block("开源贡献 & 落地", [
    "MIT 开源：index.html 原型 / SYSTEM_PROMPT(+MINOR) / 知识库索引 / 评测红队套件 / 双版本与品牌设计文档；公开升维框架+规划中的四层审核+适龄分级范式。",
